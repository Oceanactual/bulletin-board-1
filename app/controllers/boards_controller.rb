class BoardsController < ApplicationController
  ...
  def show
    ...
    @board_page = params.fetch("path_id")
    @the_board = Board.where({:id => @board_page}).first
    @list_of_posts = Post.where({:board_id => @board_page})
...
def create
  p = Post.new
  p.title = params.fetch("the_post_title")
  p.body = params.fetch("the_post_body")
  p.expires_on = params.fetch("the_post_expires")
  p.board_id = params.fetch("the_board_id")
  if p.body != ""
    p.save
  end
  redirect_to("/boards/#{p.board_id}")
end
